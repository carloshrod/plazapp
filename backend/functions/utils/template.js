...
// ********** Notificaciones de término de contrato **********
export const docNotification = (data) => {
  const { userDisplayName, docType, docName } = data;
  ...
  return `
          ...
          <body>
          <div class="container">
              <h3>Señor(a) Administrador(a),</h3>
              <section class="content">
              <p>El locatario <strong>${userDisplayName}</strong> acaba de subir un documento.</p>
              <p class="description">Tipo de documento: ${`${docType
                .charAt(0)
                .toUpperCase()}${docType.slice(1)}`}</p>
              <p class="description">Nombre del documento: ${docName}</p>
              <p>Cordialmente,</p>
              <p>El equipo de Plazapp</p>
          </div>
          </body>
